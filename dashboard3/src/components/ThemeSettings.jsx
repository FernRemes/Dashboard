        ...
        <div className = "flex-col border-t-1 border-color p-4 ml-4">
          ...
          <div className = "flex gap-3 "> 
            {themeColors.map((item, index) => (
              <TooltipComponent key = {index} content = {item.name} target = {item.name} placement = "TopCenter">
                <div className = "reltive mt-2 cursor-pointer flex-gap-5 items-center">

                <button type = "button" className = "w-11 h-11 rounded-full bg-gray-100 hover:bg-gray-200 dark:hover:bg-gray-600" style = {{backgroundColor: item.color}} onClick = {() => {setColor(item.color)}} >
